...
class ModelConfigTests(unittest.TestCase):
    def test_load_config_uses_persisted_model_settings_and_env_overrides(self):
        ...
        keys = ["APP_HOME", "LLM_PROVIDER", "LLM_MODEL", "OPENAI_BASE_URL"]
    ...
    def test_load_config_reads_bounded_onionclaw_settings(self):
        from ares.config.loader import load_config, save_onionclaw_config

        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            save_onionclaw_config(
                home=home,
                enabled=True,
                repo_path="/opt/onionclaw",
                python_bin="/usr/bin/python3",
            )
            cfg = load_config(home)

        self.assertTrue(cfg.onionclaw.enabled)
        self.assertEqual(cfg.onionclaw.repo_path, "/opt/onionclaw")
        self.assertEqual(cfg.onionclaw.python_bin, "/usr/bin/python3")
        self.assertEqual(cfg.onionclaw.env_path, str(home / "integrations" / "onionclaw" / ".env"))
        self.assertEqual(cfg.onionclaw.db_path, str(home / "integrations" / "onionclaw" / "sicry.db"))
    ...
    def test_load_config_loads_missing_values_from_home_dotenv_without_overriding_process_env(self):
        ...
        keys = ["APP_HOME", "LLM_MODEL", "OPENAI_BASE_URL", "OPENAI_API_KEY", "OPENAI_API_KEY"]
        old_values = {key: os.environ.get(key) for key in keys}
        try:
            with tempfile.TemporaryDirectory() as tmp:
                home = Path(tmp)
                ...
                os.environ["APP_HOME"] = tmp
                os.environ["LLM_MODEL"] = "from-process"
                for key in ("OPENAI_BASE_URL", "OPENAI_API_KEY", "OPENAI_API_KEY"):
                    os.environ.pop(key, None)
